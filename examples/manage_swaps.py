#!/usr/bin/env python3
"""
manage_swaps.py  —  Multi-instrument Pricer and Trade Manager
=============================================================
...
from models import (TradeBase, BaseLeg, FixedLeg, FloatLeg, OptionLeg, EquityLeg, CreditLeg, EquityOptionLeg,
...
def run_pricing(n_irs: int = 50, n_bonds: int = 50, n_opts: int = 50,
    ...
    for it in range(1, max_iter + 1):
        ...
        curr_npv = np.array([r.get("swap_npv", float("nan")) for r in result])
...
def print_results(records_or_df, project_dir: str) -> None:
    ...
    print(f"  Total trades      : {len(all_recs):>5}  "
          f"(IRS:{len(irs_recs)}  Bonds:{len(bond_recs)}  Swaptions:{len(opt_recs)}  "
          f"EqSwap:{len(eq_recs)}  CDS:{len(cds_recs)}  EqOpt:{len(eqopt_recs)})")
